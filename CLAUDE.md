...
assets/fonts/
  Rubik-Regular.ttf              # UI body text + seat names
  TitanOne-Regular.ttf           # Chunky display weight for card ranks
...
- **Suit rendering**: real Unicode symbols (♥♦♣♠) rendered with `NotoSansSymbols2-Regular.ttf`; ranks use the chunky display font `TitanOne-Regular.ttf` (Rubik only ships as a variable font and Bevy renders just its default/regular axis, so a heavy static face is used for the ranks instead). Two separate `Text2dBundle` children per card so each glyph uses the right font. UI body text + seat names use `Rubik-Regular.ttf`; HUD/headers/tags/turn-prompt use the pixel font `Silkscreen-Regular.ttf`.
